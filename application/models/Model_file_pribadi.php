        ...
        if (!$this->db->table_exists($this->tabel_folder)) {
            $sql = "
                CREATE TABLE `{$this->tabel_folder}` (
                    ...
                    FOREIGN KEY (`id_parent`) REFERENCES `{$this->tabel_folder}` (`id_folder`) ON DELETE CASCADE
        ...
        if (!$this->db->table_exists($this->tabel_file)) {
            $sql = "
                CREATE TABLE `{$this->tabel_file}` (
                    ...
                    FOREIGN KEY (`id_pengguna`) REFERENCES `pengguna` (`id_pengguna`) ON DELETE CASCADE,
                    FOREIGN KEY (`id_folder`) REFERENCES `{$this->tabel_folder}` (`id_folder`) ON DELETE SET NULL
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
            ";
