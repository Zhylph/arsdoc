        ...
        if ($form_submitted) {
            ...
            foreach ($data['field_template'] as $field) {
                ...
                if ($field['tipe_field'] === 'file' && $is_required) {
                    ...
                    $field_value = $this->input->post('field_' . $field['nama_field']);
    ...
    private function _proses_submission($template, $field_template) {
        ...
        foreach ($field_template as $field) {
            ...
            if ($field['tipe_field'] === 'file') {
                ...
                if (!empty($_FILES['field_' . $field['nama_field']]['name'])) {
                    $upload_result = $this->_upload_file('field_' . $field['nama_field'], $template);
                ...
                $field_value = $this->input->post('field_' . str_replace(' ', '_', $field['nama_field']));
                
                
                log_message('debug', 'Field: ' . $field['nama_field'] . ', Value: ' . $field_value);
            }
        ...
        log_message('debug', 'Saving submission data: ' . print_r($data_submission, true));
        log_message('debug', 'Saving field data: ' . print_r($data_field, true));
    ...
    private function _proses_edit_submission($submission, $field_template) {
        ...
        foreach ($field_template as $field) {
            ...
            if ($field['tipe_field'] === 'file') {
                ...
                if (!empty($_FILES['field_' . str_replace(' ', '_', $field['nama_field'])]['name'])) {
                    
                    $template_data = $this->Model_template_dokumen->ambil_template_by_id($submission['id_template']);
                    $upload_result = $this->_upload_file('field_' . str_replace(' ', '_', $field['nama_field']), $template_data);
                    if ($upload_result['success']) {
                        $field_value = $upload_result['file_name'];
                        $uploaded_files[] = $upload_result['file_name'];
                ...
                log_message('debug', 'Edit - Field: ' . $field['nama_field'] . ', Value: ' . $field_value);
            }
        ...
        log_message('debug', 'Edit - Saving field data: ' . print_r($data_field, true));
